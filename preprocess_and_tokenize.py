...
def tokenize(line):
    """Tokenizes a line of text, separates out the sentences
    in the line (if more than one), and returns tokenized
    sentence(s) as list."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Read in a Gigaword file and preprocess it, \
                outputting to standard output.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('infile',
        help='Gigaword file; can be piped in via cat, more, etc.',
    ...
    if args.infile.isatty():
        msg = dedent('''You are running this script interactively. Press CTRL-D at the
        start of a blank line to signal the end of your input. For help,
        run it with --help''').strip()
